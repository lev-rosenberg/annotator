...
export function D3Annotator(props: annotatorProps) {
  ...
  const [polylinePoints, setPoints] = useState<Point[]>([]);
  ...
  useLayoutEffect(() => {
    ...
    svg.on("mousedown", function (e) {
      if (props.isDrawing && isWithinImage(e.x, e.y, scale)) {
        controls.handleDrawPolylineOnClick(
          e,
          props.svgElement,
          polylinePoints,
          setPoints,
  ...
  }, [dragging, zooming, props.dialogueOpen]);
  ...
  const handlePolylineMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (props.isDrawing && polylinePoints.length >= 1) {
        ...
        const [x, y] = controls.getProportionalCoords(
          offsetX,
          offsetY,
          props.svgElement.current
        );
        ...
        setPoints((prevPoints) => {
          ...
          if (controls.closingPoly(newVertex, polylinePoints, t, scale)) {
    ...
    [polylineLen, polylinePoints, props.isDrawing, props.svgElement, scale, t]
  );
